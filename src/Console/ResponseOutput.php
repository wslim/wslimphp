...
class ResponseOutput
{
    ...
    /** @var int */
    private $verbosity = Response::VERBOSITY_NORMAL;
    ...
    public function write($messages, $newline = false, $type = Response::OUTPUT_NORMAL, $stream = null)
    {
        if (Response::VERBOSITY_QUIET === $this->getVerbosity()) {
            return;
        }
        ...
        foreach ($messages as $message) {
            switch ($type) {
                case Response::OUTPUT_NORMAL:
                    $message = $this->formatter->format($message);
                    break;
                case Response::OUTPUT_RAW:
                    break;
                case Response::OUTPUT_PLAIN:
    ...
    public function renderException(\Exception $e)
    {
        ...
        do {
            ...
            $this->write($messages, true, Response::OUTPUT_NORMAL, $stderr);

            if (Response::VERBOSITY_VERBOSE <= $this->getVerbosity()) {
                $this->write('<comment>Exception trace:</comment>', true, Response::OUTPUT_NORMAL, $stderr);
                ...
                for ($i = 0, $count = count($trace); $i < $count; ++$i) {
                    ...
                    $this->write(sprintf(' %s%s%s() at <info>%s:%s</info>', $class, $type, $function, $file, $line), true, Response::OUTPUT_NORMAL, $stderr);
                }

                $this->write('', true, Response::OUTPUT_NORMAL, $stderr);
                $this->write('', true, Response::OUTPUT_NORMAL, $stderr);
            }
