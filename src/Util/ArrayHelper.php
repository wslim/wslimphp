...
/**
 * The ArrayHelper class refence the Joomla ArrayHelper.
 * 
...
{
	/**
	 * convert to array integer values
	 *
	 * @param   mixed  $array    The source array to convert
	...
	/**
	 * from array to raw string 转换数组为raw 直接string格式, 可以 eval, 过程可逆, 适合html模板输出并动态执行之
	 * the other way is serialize, but can't dynamic eval
	 * 
	 * @param  array $data 数组
	 * @param  bool  $stripslashes 是否 stripslashes 处理
	 * $param  bool  $line 是否换行
	 * @return string
	 */
	static public function toRaw($data, $stripslashes=true, $line=false) 
	{
	    $str     = $data;
	    $linestr = $line ? PHP_EOL : '';
	    if (is_array($data)) {
	        ...
	        foreach ($data as $key=>$val) {
	            if (is_array($val)) {
	                $str .= "'$key'=>". static::toRaw($val) . ",";
	            } else {
	                if (strpos($val, '$') === 0) {
	                    $str .= "'$key' => isset($val) ? $val : null,";
	                } else {
	                    $str .= "'$key' => '" . static::addslashes($val)."',";
	                }
	            }
	            $str .= $linestr;
	        }
	        
	        return rtrim($str, ',' . $linestr) . ']';
	    }
	...
	/**
	 * from raw string to array 将字符串转换为数组
	 *
	 * @param	string	$data	字符串
	 * @return	array
	 */
	static public function fromRaw($data) 
	{
	...
	/**
	 * addslashes to the array or string
	 *
	...
	static public function addslashes($data)
	{
	    ...
	    foreach($data as $key => $val) {
	        $data[$key] = static::addslashes($val);
	    }
	...
	static public function stripslashes($data) 
	{
	    ...
	    foreach($data as $key => $val) {
	        $data[$key] = static::stripslashes($val);
	    }
	...
	/**
	 * use: 根据键名获取 options 的一项，如不存在该键名，则随机返回一项，并加入同义词
	 * 
	 * 解析设置
     * 1 存在该键名的配置
     * 1.1  如果值为字串，认为是同义词配置，取其值作为键的配置，如 'read' => 'master'
     * 1.2  如果值是数组且数组键名是数字，认为是同义词配置，取其随机一个值作为键，比如 'write'=>['master1', 'master2']，会从write配置中取随机的一个
     * 1.3  其他直接取该键名的配置，如 'master' => [...]
     * 2 如果不存在该键名的配置，则随机取一个返回
     * 
	 * @param  string $key
	 * @return array ['itemKey' => itemValue]
	 */
	static public function getItemArray(& $array, $key=null)
	{
	    if (is_null($array)) return null;
	    
	    $realName = $options = null;
	    if (isset($key) && isset($array[$key])) {
	        if (is_string($array[$key])) {
	            $realName = $array[$key];
	            $options  = & $array[$realName];
	        } elseif (is_array($array[$key]) && isset($array[$key][0]) ) {
	            $realName = $array[$key][array_rand($array[$key], 1)]; 
	            $options = & $array[$realName];
	        } else {
	            $options = & $array[$key];
	        }
	        ...
	        if (isset($array['default'])) {
	            ...
	            // 取数组的随机一项
	            //$realName = array_rand($injectOptions, 1);
	            
	            // 取数组的第一项
	            $realName = array_keys($array)[0];
	            $options = $array[$realName];
	        }
	    }
	    
	    return [$realName => $options];
	}
	
	/**
	 * handle array value by callable
	 * @param  array    $array
	 * @param  callable $callback
	 * @return array
	 */
	static public function callback($array, $callback)
	{
	    if (is_array($array)) foreach ($array as $k=>$v) {
	        if (is_array($v)) {
	            $array[$k] = static::callback($v, $callback);
	        } else {
	            $array[$k] = $callback($v);
	        }
	    }
	...
	/**
	 * array key to lowercase
	 * @param  array $array
	 * @return array
	 */
	static public function toLowerCaseKey(array $array)
	{
	    $res = [];
	    if ($array) {
	        foreach ($array as $k  => $v) {
	            if (is_array($v)) {
	                $v = static::toLowerCaseKey($v);
	            }
	            $res[strtolower($k)] = $v;
	        }
	    }
