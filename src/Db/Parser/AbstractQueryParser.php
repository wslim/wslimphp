        ...
        if(is_array($val)) {                                    // 1 值是数组形式 'name'=>[ ]
            if (isset($val[1]) && is_string($val[0])) {                  // 1.1 值是数组，且第一个元素为字串，则解析为操作符 'name'=>['operator', 'name']
                ...
                if (preg_match('/^(eq|neq|gt|egt|lt|elt|<>|>|<|>=|<=|=)$/', $exp)) {          // 比较运算 'name'=>['>=', 'value']
                    ...
                    if (!is_numeric($val[1]) && is_string($val[1])) {
                        $val[1] = preg_split('/\s+/', $val[1]);
                    }
                    
                    if (is_array($val[1])) {
                        $likeLogic  =   isset($val[2]) ? strtoupper($val[2]) : null;
                        if(!$likeLogic || !in_array($likeLogic, array('AND','OR','XOR'))){
                            $likeLogic = 'OR';
                        }
                        $like       =   array();
                        ...
                        $val[1]   = trim($val[1], '%');
                        $whereStr .= $key . ' ' . static::$operators[$exp] . ' ' . $this->formatValue('%' . $val[1] . '%');
                    }
