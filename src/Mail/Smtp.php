...
class Smtp
{
    ...
    public function connect($timeout = self::TIMEOUT, $test = false)
    {
        ...
        if (!$this->socket || strlen($errstr) > 0 || $errno > 0) {
            //throw exception
            Exception::instance()->setMessage(Exception::SERVER_ERROR, $host.':'.$this->port)->trigger();
        }
        ...
        if (!$this->call('EHLO '.$server_host, 250)
            && !$this->call('HELO '.$server_host, 250)) {
                $this->disconnect();
                //throw exception
                Exception::instance()
            ...
            if (!$this->call('AUTH LOGIN', 250, 334)) {
                $this->disconnect();
                //throw exception
                Exception::instance(Exception::LOGIN_ERROR)->trigger();
            }
            
            if (!$this->call(base64_encode($this->username), 334)) {
                $this->disconnect();
                //throw exception
                Exception::instance()->setMessage(Exception::LOGIN_ERROR);
            }
    ...
    public function reply($messageId, $topic = null, array $headers = array())
    {
        ...
        if (!$this->call('DATA', 354)) {
            $this->disconnect();
            //throw exception
            Exception::instance(Exception::SMTP_DATA)->trigger();
        }
     ...
     * Sends an email
     *
     * @param  array  $headers Custom headers ['Date'=>, 'Subject'=>, 'From'=>, 'To'=>, 'Body'=>]
     *
     * @return array headers
     * Array
     (
         [Date] => Wed, 3 Jan 2017 09:54:18 +0800
         [Subject] =>
         [From] => <sweeper@aliyun.com>
         [To]   => <wslimphp@aliyun.com>
         [Message-ID] => <c2c62314b9b61b541033573dad04c657.eden@aliyun.com>
         [Thread-Topic] =>
         [Reply-To] => <sweeper@aliyun.com>
     )
     */
    public function send(array $headers = array())
    {
        ...
        //add from
        if (!$this->socket || !$this->call('MAIL FROM:<' . $this->username . '>', 250, 251)) {
            $this->disconnect();
            //throw exception
            Exception::instance()
            ->setMessage(Exception::SMTP_ADD_EMAIL)
    ...
    protected function addAttachmentBody(array $body)
    {
        foreach ($this->attachments as $attachment) {
            ...
            $mime   = $mime ? $mime : MimeType::getMimeType($name);
    ...
    protected function getHeaders(array $customHeaders = array())
    {
        ...
        $headers['Message-ID']  = '<'.md5(uniqid(time())).'.wslimphp@'.$suffix.'>';
