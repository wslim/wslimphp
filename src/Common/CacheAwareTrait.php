...
trait CacheAwareTrait
{
    ...
    /**
     * get cache class name, this will as cache group
     * @return string
     */
    ...
    public function cacheCall($key=null, $callable=null, $args=null, $cache_mode = Cache::CACHE_AUTO)
    {
        ...
        if (!$key) {
            ...
            $key = StringHelper::toUnderscoreVariable(static::getCacheClassName()) . '/';
            $key .= (is_string($function) ? StringHelper::toUnderscoreVariable($function) . '/' : '') . md5(serialize([$function, $args]));
        }
