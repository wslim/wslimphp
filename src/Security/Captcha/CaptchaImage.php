<?php
namespace Wslim\Security\Captcha;

/**
 * 验证码图形类，render()传递一个code值进行显示，code值为数字和大小写字母
 * 
 * @example
 * <p>
 * header("Expires: -1");<br>
 * header("Cache-Control: no-store, private, post-check=0, pre-check=0, max-age=0", FALSE);<br>
 * header("Pragma: no-cache");<br>
 * header('Content-type: image/png');
 * 
 * $captcha = new CaptchaImage();<br>
 * $captcha->render('12345abc');<br>
 * </p>
 * 
...
class CaptchaImage 
{
	...
	{
	    $this->datapath = realpath(__DIR__);
	}
	...
	/**
	 * render image, if $filename is set then save to file
	 * 
	 * @param  string $code 1234
	 * @param  string $filename if set then save to this file
	 * 
	 * @return void
	 */
	public function render($code, $filename=null)
	{
	    if(function_exists('imagecreate') && function_exists('imagecolorset') && function_exists('imagecopyresized') &&
            ...
            exit('gd extension is not setting.');
        }
